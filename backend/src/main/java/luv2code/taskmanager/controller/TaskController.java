...
public class TaskController {
    ...
    /**
     * Helper method to securely extract and validate userId from authentication context
     */
    private String extractAndValidateUserId(Authentication authentication) {
        if (authentication == null || !authentication.isAuthenticated()) {
            log.warn("Unauthorized access attempt: authentication is null or not authenticated");
            throw new UnauthorizedException("User must be authenticated to access tasks");
        }

        Object principal = authentication.getPrincipal();
        if (principal == null || !(principal instanceof String)) {
            log.warn("Unauthorized access attempt: invalid principal in authentication");
            throw new UnauthorizedException("Invalid authentication context");
        }

        String userId = (String) principal;
        if (userId == null || userId.trim().isEmpty()) {
            log.warn("Unauthorized access attempt: userId is empty or null");
            throw new UnauthorizedException("User ID cannot be empty");
        }
    ...
    public ResponseEntity<ApiResponse<TaskDTO>> createTask(@RequestBody TaskCreateRequest request, Authentication authentication) {
        ...
        String userId = extractAndValidateUserId(authentication);
    ...
    public ResponseEntity<ApiResponse<List<TaskDTO>>> getAllTasks(Authentication authentication) {
        ...
        String userId = extractAndValidateUserId(authentication);
        log.debug("Retrieving all tasks for user: {}", userId);
    ...
    public ResponseEntity<ApiResponse<TaskDTO>> getTaskById(@PathVariable String taskId, Authentication authentication) {
        ...
        String userId = extractAndValidateUserId(authentication);
        log.debug("User {} requesting access to task: {}", userId, taskId);
    ...
    public ResponseEntity<ApiResponse<TaskDTO>> updateTask(@PathVariable String taskId, @RequestBody TaskCreateRequest request, Authentication authentication) {
        ...
        String userId = extractAndValidateUserId(authentication);
        log.debug("User {} updating task: {}", userId, taskId);
    ...
    public ResponseEntity<ApiResponse<String>> deleteTask(@PathVariable String taskId, Authentication authentication) {
        ...
        String userId = extractAndValidateUserId(authentication);
        log.debug("User {} deleting task: {}", userId, taskId);
